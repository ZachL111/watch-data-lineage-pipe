...
`watch-data-lineage-pipe` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies lineage behavior through node-edge fixtures, with cycle and reachability reports and local-only command execution.
...
The score is deterministic. The current threshold is 174. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
